...
- 方式二
  ...
  ```shell
  [root@master02 ~]# rpm -qa | grep kernel
  ...
  ```shell
  [root@master02 ~]# grub2-set-default  0
  
  # 上面如果不生效使用下面方式设置
  
  [root@master02 ~]# grubby --set-default /boot/kernel-ml-6.2.7-1.el7.elrepo.x86_64
  ...
  ```shell
  [root@master02 ~]# grubby --default-kernel
